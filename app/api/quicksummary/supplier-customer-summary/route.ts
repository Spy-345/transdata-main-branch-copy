...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (dbError instanceof Error) {
        if (dbError.message.includes("timeout")) {
          throw new Error(
            "Database query timed out. Please try a more specific search term."
          );
    ...
    // console.log(
    //   `🔍 DEBUG: Found ${records.length} records for query "${searchQuery}"`
    // );
    ...
    const errorDetails = {
      ...
      analysisType:
        request.nextUrl.searchParams.get("type") || "supplier-customer",
    };
...
function generateDynamicOthersLabel(
  parentName: string,
  type: "supplier" | "country"
): string {
...
async function processSupplierCustomerData(
  records: any[],
  limit: number,
  dateRange: string
  ...
  records.forEach((record) => {
    ...
    let customerName = normalizeCompanyName(
      record.buyer_name || "Unknown Customer"
    );
    ...
    // if (value > 1000000) {
    //   console.log(
    //     `🔍 DEBUG: High value detected: ${value} for supplier ${supplierName}`
    //   );
    // }
  ...
  const totalMarketValue = suppliersArray.reduce(
    (sum, supplier) => sum + supplier.totalValue,
    0
  );
  const totalMarketShipments = suppliersArray.reduce(
    (sum, supplier) => sum + supplier.totalShipments,
    0
  );
  ...
  //   console.log(
  //     `🔍 DEBUG: Top supplier: ${suppliersArray[0].name} with value: ${suppliersArray[0].totalValue}`
  //   );
  ...
  const suppliersWithMarketShare = top5Suppliers.map((supplier) => {
      ...
      .map(([name, data]) => ({
        ...
        percentage:
          supplier.totalValue > 0
            ? (data.value / supplier.totalValue) * 100
            : 0,
      }))
    ...
    if (supplier.customers.size > 5) {
      const top5CustomerValue = customerArray.reduce(
        (sum, customer) => sum + customer.value,
        0
      );
      const top5CustomerShipments = customerArray.reduce(
        (sum, customer) => sum + customer.shipments,
        0
      );
      const othersCustomerValue = supplier.totalValue - top5CustomerValue;
      const othersCustomerShipments =
        supplier.totalShipments - top5CustomerShipments;
      ...
      if (othersCustomerValue > 0) {
        customerArray.push({
          ...
          percentage:
            supplier.totalValue > 0
              ? (othersCustomerValue / supplier.totalValue) * 100
              : 0,
        });
      }
    }
    ...
    return {
      supplier: {
        ...
        marketShare:
          totalMarketValue > 0
            ? (supplier.totalValue / totalMarketValue) * 100
            : 0,
  ...
  return {
    ...
    summary: {
      ...
      averageValue:
        totalMarketShipments > 0 ? totalMarketValue / totalMarketShipments : 0,
...
async function processGeographicData(
  ...
  const totalMarketValue = countriesArray.reduce(
    (sum, country) => sum + country.totalValue,
    0
  );
  const totalMarketShipments = countriesArray.reduce(
    (sum, country) => sum + country.totalShipments,
    0
  );
  ...
  const countriesWithMarketShare = top5Countries.map((country) => {
    const importerArray = Array.from(country.importers.entries())
      .map(([name, data]) => ({
        ...
        shipments: data.shipments,
        percentage:
          country.totalValue > 0 ? (data.value / country.totalValue) * 100 : 0,
      }))
      .sort((a, b) => b.value - a.value)
      .slice(0, 5);

    if (country.importers.size > 5) {
      const top5ImporterValue = importerArray.reduce(
        (sum, importer) => sum + importer.value,
        0
      );
      const top5ImporterShipments = importerArray.reduce(
        (sum, importer) => sum + importer.shipments,
        0
      );
      const othersImporterValue = country.totalValue - top5ImporterValue;
      const othersImporterShipments =
        country.totalShipments - top5ImporterShipments;
        ...
        importerArray.push({
          ...
          percentage:
            country.totalValue > 0
              ? (othersImporterValue / country.totalValue) * 100
              : 0,
        });
      }
    }
    ...
    return {
      country: {
        ...
        marketShare:
          totalMarketValue > 0
            ? (country.totalValue / totalMarketValue) * 100
            : 0,
